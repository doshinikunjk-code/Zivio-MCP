...
// ── RAILWAY API — auto-persist keys to VALID_KEYS env var ────
const RAILWAY_TOKEN   = process.env.RAILWAY_TOKEN   || 'ea27d0d3-080a-4a22-8310-bf3f4c91be51';
const RAILWAY_PROJECT = process.env.RAILWAY_PROJECT || 'f1f3c5a4-0607-4f73-a541-394228ecba9f';
const RAILWAY_SERVICE = process.env.RAILWAY_SERVICE || '581a6433-3c7f-457b-881c-2a1983947a92';
const RAILWAY_ENV     = process.env.RAILWAY_ENV     || '15bf0528-daaf-4284-9738-6d512c67f75c';

async function persistKeysToRailway(keySet) {
  try {
    const keysValue = [...keySet].join(',');
    const mutation = `
      mutation {
    ...
    if (data.errors) {
      console.error('[RAILWAY] Failed to persist keys:', JSON.stringify(data.errors));
    } else {
      console.log('[RAILWAY] Keys persisted to VALID_KEYS env var successfully');
    }
  } catch(e) {
    console.error('[RAILWAY] Persist error:', e.message);
  }
}
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed' || event.type === 'invoice.paid') {
      ...
      if (email) {
        ...
        saveKeys(validKeys);
        persistKeysToRailway(validKeys);
