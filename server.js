...
app.post('/tools/gap_scan', authMiddleware, async (req, res) => {
  try {
    ...
    if (!data.tickers?.length) {
      return res.json({ tool: 'gap_scan', results: [], message: 'No gainers data available' });
    }
    ...
    const results = data.tickers
      ...
      .map(t => {
        ...
        const float = t.shareClassSharesOutstanding || 0;
        ...
        return {
          ...
          low: t.day?.l || 0,
          float: float,
          float_label: float < 5e6 ? 'NANO' : float < 15e6 ? 'MICRO' : float < 50e6 ? 'SMALL' : 'LARGE'
        };
      });
