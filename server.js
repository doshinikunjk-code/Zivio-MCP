...
async function loadKeysFromPg() {
  try {
    const client = await getPgClient();
    if (!client) return new Set();
    const result = await client.query('SELECT key FROM api_keys WHERE active = TRUE');
    const keys = new Set(result.rows.map(r => r.key));
    console.log('[PG] Loaded', keys.size, 'keys from PostgreSQL');
    return keys;
  } catch(e) {
    console.error('[PG] Load keys error:', e.message);
    return new Set();
  }
}

async function saveKeyToPg(key, email = '') {
  try {
    const client = await getPgClient();
    if (!client) return;
    await client.query(
      'INSERT INTO api_keys (key, email) VALUES ($1, $2) ON CONFLICT (key) DO NOTHING',
      [key, email]
    );
    console.log('[PG] Key saved to PostgreSQL');
  } catch(e) {
    console.error('[PG] Save key error:', e.message);
  }
}

async function revokeKeyInPg(key) {
  try {
    const client = await getPgClient();
    if (!client) return;
    await client.query('UPDATE api_keys SET active = FALSE WHERE key = $1', [key]);
    console.log('[PG] Key revoked in PostgreSQL');
  } catch(e) {
    console.error('[PG] Revoke key error:', e.message);
  }
}

// Load keys from PostgreSQL + seed from env
let validKeys = new Set();
(process.env.VALID_KEYS || '').split(',').map(k => k.trim()).filter(Boolean).forEach(k => validKeys.add(k));

// Async init — load keys from PG on startup
async function initKeys() {
  const pgKeys = await loadKeysFromPg();
  pgKeys.forEach(k => validKeys.add(k));
  console.log('[KEYS] Total keys loaded:', validKeys.size);
}
...
app.get('/tools/gap_scan/:apikey', async (req, res) => {
  ...
  try {
    const PKEY = process.env.POLYGON_KEY || '';
...
app.get('/tools/sec_alert/:apikey', async (req, res) => {
  if (!validKeys.has(req.params.apikey)) return res.status(401).json({ error: 'Invalid API key' });
  try {
    const SKEY = process.env.SEC_API_KEY || '';
...
async function sendKeyEmail(email, apiKey) {
  ...
  "mcpServers": {
    ...
    const brevoResult = await brevoResp.json().catch(() => ({}));
    if (brevoResp.ok) {
      console.log(`[BREVO] Email sent to ${email} — ID: ${brevoResult.messageId}`);
    } else {
      console.error(`[BREVO] Send failed:`, JSON.stringify(brevoResult));
    }
  } catch (e) {
...
// This is what Claude Desktop and MCP clients connect to
app.post('/mcp', async (req, res) => {
  ...
  console.log('[MCP] Auth attempt — x-api-key:', xApiKey ? 'present' : 'missing', '| auth header:', authHeader ? authHeader.slice(0,20) : 'missing', '| valid:', validKeys.has(apiKey));
  if (!apiKey || !validKeys.has(apiKey)) {
    return res.status(401).json({ error: 'Invalid or missing API key', hint: 'Send as x-api-key header or Bearer token' });
  }
  ...
  // Tool: gap_scan
  server.tool('gap_scan', 'Scan for pre-market gap stocks', {
    min_gap_pct: z.number().default(5),
    min_volume: z.number().default(100000),
    max_price: z.number().default(60),
    limit: z.number().default(10)
  }, async ({ min_gap_pct, min_volume, max_price, limit }) => {
    try {
      const url = `https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/gainers?apiKey=${PKEY}`;
      const resp = await fetch(url);
      const data = await resp.json();
      const results = (data.tickers || [])
        .filter(t => {
          const p = t.lastTrade?.p || t.day?.c || 0;
          const pc = t.prevDay?.c || 0;
          const v = t.day?.v || 0;
          const g = pc > 0 ? ((p - pc) / pc * 100) : 0;
          return p >= 1 && p <= max_price && g >= min_gap_pct && v >= min_volume;
        })
        .map(t => {
          const p = t.lastTrade?.p || t.day?.c || 0;
          const pc = t.prevDay?.c || 0;
          const v = t.day?.v || 0;
          const g = pc > 0 ? ((p - pc) / pc * 100) : 0;
          return { ticker: t.ticker, price: +p.toFixed(2), gap_pct: +g.toFixed(1), volume: v, vwap: t.day?.vw || 0 };
        })
        .sort((a, b) => b.gap_pct - a.gap_pct)
        .slice(0, limit);
  ...
  // Tool: sec_alert
  server.tool('sec_alert', 'Fetch latest SEC filings with AI summary', {
    ticker: z.string().optional(),
    form_type: z.string().default('8-K'),
    hours: z.number().default(24),
    limit: z.number().default(5)
  }, async ({ ticker, form_type, hours, limit }) => {
    try {
      let query = `formType:"${form_type}" AND filedAt:{now-${hours}h TO now}`;
      if (ticker) query = `ticker:"${ticker.toUpperCase()}" AND ` + query;
      const resp = await fetch('https://api.sec-api.io?token=' + SKEY, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ query: { query_string: { query } }, from: '0', size: String(limit), sort: [{ filedAt: { order: 'desc' } }] })
      });
      const data = await resp.json();
      const results = (data.filings || []).map(f => ({ ticker: f.ticker, company: f.companyName, form: f.formType, filed: f.filedAt, description: f.description }));
      return { content: [{ type: 'text', text: JSON.stringify({ count: results.length, results }, null, 2) }] };
    } catch(e) {
      return { content: [{ type: 'text', text: 'Error: ' + e.message }] };
    }
  ...
  // Tool: news_sentiment
  server.tool('news_sentiment', 'Get news headlines with sentiment scoring', {
    ticker: z.string(),
    limit: z.number().default(10)
  }, async ({ ticker, limit }) => {
    try {
      const url = `https://api.polygon.io/v2/reference/news?ticker=${ticker.toUpperCase()}&limit=${limit}&apiKey=${PKEY}`;
      const resp = await fetch(url);
      const data = await resp.json();
      const bullish = ['beats','surge','jump','soar','record','upgrade','buy','strong','gain','rally'];
      const bearish = ['miss','fall','drop','plunge','downgrade','sell','weak','loss','crash','cut'];
      const results = (data.results || []).map(a => {
        ...
        return { headline: a.title, publisher: a.publisher?.name, sentiment: b > r ? 'BULLISH' : r > b ? 'BEARISH' : 'NEUTRAL', published: a.published_utc };
      });
      const overall = results.filter(r => r.sentiment === 'BULLISH').length > results.filter(r => r.sentiment === 'BEARISH').length ? 'BULLISH' : 'BEARISH';
      return { content: [{ type: 'text', text: JSON.stringify({ ticker: ticker.toUpperCase(), overall_sentiment: overall, count: results.length, results }, null, 2) }] };
    } catch(e) {
      return { content: [{ type: 'text', text: 'Error: ' + e.message }] };
    }
  });
  ...
  // Tool: insider_scan
  server.tool('insider_scan', 'Scan for recent insider buying activity', {
    ticker: z.string().optional(),
    limit: z.number().default(20)
  }, async ({ ticker, limit }) => {
    try {
      let url = `https://api.sec-api.io/insider-trading?token=${SKEY}&transactionType=P-Purchase&limit=${limit}&sort=-transactionValue`;
      if (ticker) url += `&ticker=${ticker.toUpperCase()}`;
      const resp = await fetch(url);
      const data = await resp.json();
      const results = (data.data || []).map(tx => ({ ticker: tx.ticker, insider: tx.reportingOwnerName, title: tx.reportingOwnerRelationship, shares: tx.numberOfShares, value: tx.totalValue, filed: tx.filedAt }));
      return { content: [{ type: 'text', text: JSON.stringify({ count: results.length, results }, null, 2) }] };
    } catch(e) {
      return { content: [{ type: 'text', text: 'Error: ' + e.message }] };
    }
  ...
  // Tool: momentum_alerts
  server.tool('momentum_alerts', 'Get real-time momentum alerts', {
    limit: z.number().default(20)
  }, async ({ limit }) => {
    try {
      const url = `https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/gainers?apiKey=${PKEY}`;
      const resp = await fetch(url);
      const data = await resp.json();
      const alerts = (data.tickers || []).slice(0, 50).filter(t => {
        const p = t.lastTrade?.p || t.day?.c || 0;
        return p >= 1 && p <= 60;
      }).map(t => {
        const p = t.lastTrade?.p || t.day?.c || 0;
        ...
        const g = pc > 0 ? ((p - pc) / pc * 100) : 0;
        const av = t.min?.av || 0;
        const rv = av > 0 ? v / av : 0;
        const types = [];
        if (g >= 20) types.push('MEGA_SQUEEZE');
        else if (g >= 10) types.push('SQUEEZE');
        if (rv >= 5) types.push('VOL_SPIKE');
        return { ticker: t.ticker, price: +p.toFixed(2), change_pct: +g.toFixed(1), relative_volume: +rv.toFixed(1), alert_types: types };
      }).filter(a => a.alert_types.length > 0).slice(0, limit);
