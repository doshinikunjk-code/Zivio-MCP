...
const POLYGON_KEY   = process.env.POLYGON_KEY   || 'lBUhL0Hy4LMMLx6djEGmJNEKEl_uSfrK';
...
// ── API KEY STORE (in-memory + env seed) ──────────────────────
// In production: replace with a simple JSON file or SQLite
let validKeys = new Set(
  (process.env.VALID_KEYS || '').split(',').map(k => k.trim()).filter(Boolean)
);
...
app.get('/tools/gap_scan/:apikey', async (req, res) => {
  ...
  req.body = { min_gap_pct: 5, min_volume: 100000, max_price: 60, limit: 10 };
  req.headers['x-api-key'] = req.params.apikey;
  // Forward to POST handler logic inline
  try {
    const PKEY = process.env.POLYGON_KEY || 'lBUhL0Hy4LMMLx6djEGmJNEKEl_uSfrK';
    const watchlist = ['AAPL','TSLA','NVDA','AMD','AMZN','MSFT','META','GOOGL','SPY','QQQ','SOFI','PLTR','RIVN','LCID','NIO','AMC','GME','MULN','SNDL','FFIE','CLOV','WKHS','NKLA','GOEV','BLNK','CHPT','PLUG','FCEL','MARA','RIOT','COIN','HUT','CLSK','BITF','CIFR','BTBT','WULF','IREN','SMCI','IONQ'].join(',');
    const url = `https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/tickers?tickers=${watchlist}&apiKey=${PKEY}`;
    const resp = await fetch(url);
    const data = await resp.json();
    if (!data.tickers?.length) return res.json({ tool: 'gap_scan', results: [], raw_status: resp.status, raw: data });
    const results = data.tickers
      .filter(t => { const p = t.lastTrade?.p||t.day?.c||0; const pc = t.prevDay?.c||0; const v = t.day?.v||0; const g = pc>0?((p-pc)/pc*100):0; return p>=1&&p<=60&&g>=5&&v>=100000; })
      .map(t => { const p = t.lastTrade?.p||t.day?.c||0; const pc = t.prevDay?.c||0; const v = t.day?.v||0; const g = pc>0?((p-pc)/pc*100):0; return { ticker: t.ticker, price: +p.toFixed(2), gap_pct: +g.toFixed(1), volume: v }; })
      .sort((a,b) => b.gap_pct - a.gap_pct).slice(0,10);
    res.json({ tool: 'gap_scan', count: results.length, results });
...
app.post('/tools/gap_scan', authMiddleware, async (req, res) => {
  try {
    ...
    const watchlist = [
      ...
      'SOFI','PLTR','RIVN','LCID','NIO','AMC','GME','MULN','SNDL','FFIE',
      'CLOV','WKHS','NKLA','GOEV','BLNK','CHPT','PLUG','FCEL','MARA','RIOT',
      'COIN','HUT','CLSK','BITF','CIFR','BTBT','WULF','IREN','SMCI','IONQ'
    ].join(',');

    const url = `https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/tickers?tickers=${watchlist}&apiKey=${POLYGON_KEY}`;
    const resp = await fetch(url);
