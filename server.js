...
const STRIPE_WEBHOOK_SECRET = process.env.STRIPE_WEBHOOK_SECRET || 'whsec_gFXjZPW2q8VyiSYW3WGczNjzposE6dxo';
const BREVO_KEY             = process.env.BREVO_KEY || '';

// ── POSTGRESQL KEY STORAGE ────────────────────────────────────
const PG_URL = process.env.DATABASE_URL || 'postgresql://postgres:kBXavPVwQheioTvXMxcQKxhemiqMWdqD@postgres-onka.railway.internal:5432/railway';
...
app.post('/webhooks/stripe', express.raw({ type: 'application/json' }), async (req, res) => {
  try {
    // Parse event — handle both Buffer and string body
    let event;
    try {
      const raw = Buffer.isBuffer(req.body) ? req.body.toString('utf8') : String(req.body);
      event = JSON.parse(raw);
    } catch(parseErr) {
      console.error('[STRIPE] Parse error:', parseErr.message, '| Body type:', typeof req.body);
